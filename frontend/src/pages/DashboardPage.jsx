...
const DashboardPage = () => {
  ...
  const quickActions = [
    {
      id: 'my-courses',
      label: t('dashboard.action.my_courses'),
      description: t('dashboard.action.my_courses_desc'),
      href: '/subjects',
    ...
    {
      id: 'exam-schedule',
      label: t('dashboard.action.exam_schedule'),
      description: t('dashboard.action.exam_schedule_desc'),
      href: '/exams',
    ...
    },
    {
      ...
      label: t('dashboard.action.ai_recommendations'),
      description: t('dashboard.action.ai_recommendations_desc'),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-slide-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white mb-1.5">
            {t('dashboard.welcome')}, <span className="text-violet-600 dark:text-violet-400 capitalize">{studentName}</span> !
          </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3 bg-violet-50 border border-violet-100 dark:bg-violet-500/10 dark:border-violet-500/20 px-4 py-2.5 rounded-2xl shadow-sm">
            ...
            <div>
              <p className="text-xs text-slate-500 dark:text-white/50 font-medium leading-none">{t('dashboard.study_streak')}</p>
              <p className="text-base font-bold text-violet-750 dark:text-white mt-1">
                {streak} {streak > 1 ? t('dashboard.days_plural') : t('dashboard.days')}
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="lg:col-span-2 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">{t('dashboard.weekly_plan_title')}</h2>
              <Link to="/ai-plan" className="text-xs text-violet-600 hover:text-violet-750 dark:text-violet-400 dark:hover:text-violet-300 font-semibold transition-colors">
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm text-slate-600 dark:text-white/60">
                  <span>{t('dashboard.plan_status')}: <span className="text-emerald-600 dark:text-emerald-400 font-bold">{t('dashboard.plan_active')}</span></span>
                  <span>{currentPlan.sessions?.length || 0} {t('dashboard.sessions_scheduled')}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl text-center border border-slate-100 dark:border-white/5">
                    ...
                    <p className="text-xs text-slate-500 dark:text-white/40 mt-1">{t('dashboard.total_study_hours')}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-xl text-center border border-slate-100 dark:border-white/5">
                    <p className="text-2xl font-bold text-slate-800 dark:text-white">
                      {currentPlan.sessions?.filter((s) => s.completed)?.length || 0}
                    </p>
                    <p className="text-xs text-slate-500 dark:text-white/40 mt-1">{t('dashboard.completed_sessions')}</p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="text-center py-8">
                <p className="text-sm text-slate-500 dark:text-white/50 mb-4">
                  {setupStatus?.has_courses
                    ? t('dashboard.no_plan_courses')
                    : t('dashboard.no_plan_setup')}
                </p>
                {setupStatus?.has_courses && setupStatus?.has_availabilities ? (
                  ...
                  >
                    {setupStatus?.has_courses ? t('dashboard.btn_set_availabilities') : t('dashboard.btn_select_courses')} →
                  </Link>
                )}
              </div>
            )}
      ...
      {/* ── Quick Actions ── */}
      <div>
        <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-4">{t('dashboard.shortcuts_title')}</h2>
