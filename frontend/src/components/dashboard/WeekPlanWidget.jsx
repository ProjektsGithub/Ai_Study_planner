    ...
    >
      ...
      <div>
        <div className="flex justify-between items-center mb-3">
          <span className="text-xs text-white/40 font-semibold uppercase tracking-wider">Current Study Plan</span>
        ...
        <div className="flex items-baseline gap-2 mb-4">
          <span className="text-3xl font-bold text-white leading-none">{hours}h</span>
          <span className="text-xs text-white/30 font-medium">of study split across {count} sessions</span>
        </div>
      </div>
        ...
        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => {
          ...
          return (
            ...
            >
              ...
              <span className={`w-1.5 h-1.5 rounded-full mt-1.5 ${hasSessions ? 'bg-emerald-400' : 'bg-transparent'}`} />
            </div>
          );
        })}
      </div>
